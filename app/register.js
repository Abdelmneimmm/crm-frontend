...
export default function RegisterScreen() {
  ...
  const handleRegister = async () => {
    if (!name || !email || !password) {
      Alert.alert('تنبيه', 'كل الحقول مطلوبة');
      return;
    }
    if (password.length < 6) {
      Alert.alert('تنبيه', 'الباسورد لازم يكون 6 حروف على الأقل');
      return;
    }
    setLoading(true);
    try {
    ...
    >
      <View style={[styles.card, { backgroundColor: theme.card }]}>
        <View style={styles.iconContainer}>
          <Ionicons name="person-add" size={50} color={theme.success} />
        </View>
        <Text style={[styles.title, { color: theme.primary }]}>حساب جديد</Text>
        <Text style={[styles.subtitle, { color: theme.textSub }]}>سجّل في CRM</Text>
        ...
        <View style={[styles.inputContainer, { borderColor: theme.border, backgroundColor: theme.inputBg }]}>
          ...
          <TextInput
            ...
            placeholder="الاسم" value={name} onChangeText={setName}
        ...
        <View style={[styles.inputContainer, { borderColor: theme.border, backgroundColor: theme.inputBg }]}>
          ...
          <TextInput
            style={[styles.input, { color: theme.text }]}
            placeholder="البريد الإلكتروني" value={email} onChangeText={setEmail}
        ...
        <View style={[styles.inputContainer, { borderColor: theme.border, backgroundColor: theme.inputBg }]}>
          ...
          <TextInput
            ...
            placeholder="كلمة المرور" value={password} onChangeText={setPassword}
            secureTextEntry={!showPass} placeholderTextColor={theme.textMuted}
          />
        ...
        <TouchableOpacity
          style={[styles.button, { backgroundColor: theme.success }, loading && styles.buttonDisabled]}
        ...
        <TouchableOpacity onPress={() => router.back()}>
          <Text style={[styles.link, { color: theme.primary }]}>عندك حساب؟ سجّل دخول</Text>
        </TouchableOpacity>
      </View>
    </KeyboardAvoidingView>
  );
